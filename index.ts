...
import { canFulfillOrder, createFulfillmentForRouzao, getShopifyOrderByNumber } from './shopify'
import type { RouzaoOrderItem, RouzaoOrders } from './types'

// Process a single shipped order
async function processShippedOrder(order: RouzaoOrderItem): Promise<void> {
  try {
    ...
    // Get Shopify order
    const shopifyOrder = await getShopifyOrderByNumber(shopifyOrderNumber)
    if (!shopifyOrder) {
    ...
    if (!(await canFulfillOrder(shopifyOrder))) {
      console.log(`[${new Date().toISOString()}] Order #${shopifyOrderNumber} cannot be fulfilled or already fulfilled`)
      return
    }
    ...
    // Create fulfillment for Rouzao location
    const success = await createFulfillmentForRouzao(
      shopifyOrder,
